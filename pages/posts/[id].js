...
// NOW WE USING API FETCH WITHOUT (Post.getInitialProps = async ({ query }) => { ) AND USES SERVER
...
// // FOR SERVER SIDE API EXTRACTION
...
// NOW WE USING API FETCH WITHOUT (Post.getInitialProps = async ({ query }) => { ) AND USES STATIc
export default function Post({ postData }) {
...
//IT IS USED TO STORE (posts) SOME SPECIFIC ID NO IN IT FOR CALLING USING API AFTER BUILD
// This function gets called at build time
export async function getStaticPaths() {
  //USED FOR DYNAMIC API FETCH
  //   // Call an external API endpoint to get posts
  ...
  //USED FOR STATIC DATA FETCH AAFTER ASSIGN BY USER ITSELF
  const paths = ['/posts/1', '/posts/2']; //These are our params to pass in static func to get specific id of data

  // We'll pre-render only these paths at build time.
  // { fallback: false } means other routes should 404.
  //   return { paths, fallback: false }; //FOR ONLY SEPECIFIC DATA AND NOT USING useRouter COMP FOR OTHER FETCHING ON RUN TIME
  return { paths, fallback: true }; //FOR ONLY SEPECIFIC DATA AND USING useRouter COMP FOR OTHER FETCHING ON RUN TIME
}

// FOR STATIC FOR BUILD TIME API EXTRACTION
// This function gets called at build time
